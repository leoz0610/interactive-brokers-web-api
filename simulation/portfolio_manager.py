...
class PortfolioManager(PortfolioManagerBase):
    """
    Manages portfolio state and tracks performance.

    This is the simulated implementation of PortfolioManagerBase.
    """
    ...
    def process_filled_order(self, order: Order) -> bool:
        ...
        if order.side == OrderSide.BUY:
            ...
            if self._cash < total_cost:
                logger.error(
                    f"Insufficient funds: need ${total_cost:.2f}, have ${self._cash:.2f}"
                )
                return False
            ...
            # Add cash from sale
            self._cash += trade_value - order.commission
        ...
        logger.info(
            ...
            f"@ ${order.filled_price:.2f}, cash: ${self._cash:.2f}"
        )
    ...
    def get_total_pnl_percent(self) -> float:
        ...
        if self._initial_cash == 0:
            return 0.0
        return (self.get_total_pnl() / self._initial_cash) * 100
            ...
            if txn.side == OrderSide.BUY:
                ...
                while remaining > 0 and symbol_cost_basis[txn.symbol]:
                    ...
                    if buy_qty <= remaining:
                        ...
                        symbol_cost_basis[txn.symbol][0] = (
                            buy_qty - remaining,
                            buy_price,
                        )
                        remaining = 0
